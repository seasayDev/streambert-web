...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function checkNewEpisodes() {
      ...
      if (typeof Notification !== "undefined" && Notification.permission === "granted") {
        ...
        try {
          new Notification("New episodes available", { body, silent: false });
        } catch {}
      }
    }
  ...
  // ── Downloads state (empty in web - no Electron downloader) ──────────────
  const [downloads, setDownloads] = useState([]);
  const [highlightDownload, setHighlightDownload] = useState(null);

  // ── Load API key from secureStorage on startup ────────────────────────────
  useEffect(() => {
    let mounted = true;
  ...
  useEffect(() => {
    ...
    document.body.style.fontSize = factor > 1 ? "110%" : factor < 1 ? "85%" : "100%";
