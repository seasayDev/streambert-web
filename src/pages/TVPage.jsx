...
export default function TVPage({
  ...
  // ── PostMessage-based iframe communication (web build) ─────────────────────
  // Replaces wv.executeJavaScript() which only works in Electron webview
  const iframeCall = useCallback(async (script, timeoutMs = 4000) => {
    const wv = webviewRef.current;
    if (!wv?.contentWindow) return null;
    return new Promise((resolve) => {
      const timer = setTimeout(() => resolve(null), timeoutMs);
      const handler = (e) => {
        if (e.data?._streambertReply) {
          clearTimeout(timer);
      ...
      wv.contentWindow.postMessage({ _streambert: true, script }, "*");
    });
  }, []);
  ...
  useEffect(() => {
    ...
    wv.addEventListener("beforeinput", handler);
    return () => wv.removeEventListener("beforeinput", handler);
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (
            ...
            result = await window.electron.queryVideoProgress(
              wv.getWebContentsId?.() ?? 0,
            );
          } else {
            result = await iframeCall(`
              (() => {
                const v = document.querySelector('video')
  ...
  useEffect(() => {
    ...
    const inject = () => {
      iframeCall(INJECT_SKIP_CONTROLS).catch(() => {});
    };

    wv.addEventListener("load", inject);
    ...
    return () => {
      wv.removeEventListener("load", inject);
      try {
        iframeCall(`
          (() => {
