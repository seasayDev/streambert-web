...
export default function SetupScreen({ onSave, onSkip }) {
  ...
  return (
    <div className="apikey-modal">
      <div className="apikey-glow" aria-hidden="true" />
      <div className="apikey-box">
        <div className="apikey-logo">
          <StreambertLogo />
        </div>
        <div className="apikey-title">STREAMBERT</div>
        <div className="apikey-tagline">
          Films &amp; Séries — votre bibliothèque, votre façon
        </div>
        <p className="apikey-sub">
          Entrez votre token TMDB{" "}
          <strong>Read Access Token</strong> (gratuit) pour commencer.
          <br />
          Rendez-vous sur{" "}
          <ExternalLink
          ...
          et copiez le <em>API Read Access Token</em> (le long JWT, pas la
          courte API Key).
          <br />
          <ExternalLink
            className="apikey-link"
          ...
          >
            Guide étape par étape pour obtenir le token
          </ExternalLink>
        </p>
        <input
          className={`apikey-input${error ? " apikey-input-error" : ""}`}
          placeholder="Collez votre TMDB Read Access Token (eyJ...)..."
          value={key}
          onChange={(e) => {
        ...
        >
          {checking ? (
            <>
              <span className="apikey-spinner" /> Vérification…
            </>
          ) : (
            <>
              <PlayIcon /> C'est parti
            </>
          )}
        </button>
